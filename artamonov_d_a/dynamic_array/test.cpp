...
TEST_CASE("DynamicArray constructors and access operators.") {
	DynamicArray empty;
	CHECK(empty.Size() == 0);
	...
	CHECK_THROWS_WITH(DA.at(DA.Size()), "Index is out of range");
}

TEST_CASE("Testing push_back.") {
	DynamicArray first{ 0, 9, 1, 7, 6 };
...
TEST_CASE("Testing clear and empty.") {
	DynamicArray first{ 0, 9, 8, 7, 6, 13, 42, 4, 2, 1 };
...
TEST_CASE("Testing resize.") {
	...
	CHECK_THROWS_WITH(third.resize(0), "The size must be a positive number");
	CHECK_THROWS_WITH(first.resize(-6), "The size must be a positive number");
}
...
TEST_CASE("Testing assign.") {
	...
	CHECK_THROWS_WITH(first.assign(0, 4), "The size must be a positive number");
	CHECK_THROWS_WITH(first.assign(-6, 13), "The size must be a positive number");
}
