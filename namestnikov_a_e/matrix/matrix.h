...
public:
    ...
    BaseMatrix(std::size_t row, std::size_t col);

    virtual void Resize(std::size_t row, std::size_t col, const T& val = {}) = 0;

    virtual T& Get(std::size_t row, std::size_t col) const = 0;
    virtual T* operator[](std::size_t row) const = 0;
protected:
    std::size_t rows = 0;
    std::size_t cols = 0;
};
...
template <typename T>
BaseMatrix<T>::BaseMatrix(std::size_t row, std::size_t col) : rows(row), cols(col)
{
...
public:
    FlatMatrix() = default;
    ~FlatMatrix();
    FlatMatrix(std::size_t row, std::size_t col, const T& val = {});
    FlatMatrix(const std::initializer_list<std::initializer_list<T>>& matrix);
    FlatMatrix(const FlatMatrix<T>& other);

    void Resize(std::size_t row, std::size_t col, const T& val = {});
...
private:
    ...
    void Rebuild(std::size_t row, std::size_t col);
    void Clear();
};

template <typename T>
FlatMatrix<T>::~FlatMatrix() {Clear();}
template <typename T>
FlatMatrix<T>::FlatMatrix(std::size_t row, std::size_t col, const T& val) : BaseMatrix<T>(row, col)
{
    data = new T[row * col];
...
template <typename T>
void FlatMatrix<T>::Resize(std::size_t row, std::size_t col, const  T& val)
{
    Rebuild(row, col);
...
template <typename T>
std::size_t FlatMatrix<T>::Rows() const {return rows;}
template <typename T>
std::size_t FlatMatrix<T>::Cols() const {return cols;}

template <typename T>
T& FlatMatrix<T>::Get(std::size_t row, std::size_t col) const
{
...
template <typename T>
T* FlatMatrix<T>::operator[](std::size_t row) const {return data + row * cols;}

template <typename T>
FlatMatrix<T> FlatMatrix<T>::operator+(const FlatMatrix<T>& rhs) const {
    if (cols != rhs.cols || rows != rhs.rows) {
        throw std::invalid_argument("You can sum matrix only with same dimensions");
    }
...
FlatMatrix<T> FlatMatrix<T>::operator-(const FlatMatrix<T>& rhs) const {
    if (cols != rhs.cols || rows != rhs.rows)
    {
        throw std::invalid_argument("You can sub matrix only with same dimensions");
    }
    FlatMatrix<T> res(*this);
    for (int i = 0; i < rows * cols; i++)
    {
...
FlatMatrix<T> FlatMatrix<T>::operator*(const FlatMatrix<T>& rhs) const {
    if (cols != rhs.cols) {
        throw std::invalid_argument("You can mult matrix only with same dimensions");
    }
...
template <typename T>
FlatMatrix<T> FlatMatrix<T>::operator+(const T& rhs) const {return *this + FlatMatrix(rows, cols, rhs);}
template <typename T>
FlatMatrix<T> FlatMatrix<T>::operator-(const T& rhs) const {return *this - FlatMatrix(rows, cols, rhs);}
template <typename T>
...
template <typename T>
bool FlatMatrix<T>::operator!=(const FlatMatrix<T>& rhs) const {return !(*this == rhs);}

template <typename T>
void FlatMatrix<T>::Rebuild(std::size_t row, std::size_t col) {
    if (row + col > 0 && row * col == 0)
    {
        throw std::invalid_argument("Matrix must be square");
    }
...
public:
    TableMatrix() = default;
    ~TableMatrix();
    TableMatrix(std::size_t row, std::size_t col, const T& val = {});
...
template <typename T>
TableMatrix<T>::~TableMatrix() {Clear();}
template <typename T>
TableMatrix<T>::TableMatrix(std::size_t row, std::size_t col, const T& val)
...
template <typename T>
void TableMatrix<T>::Resize(std::size_t row, std::size_t col, const T& val)
{
    Rebuild(row, col);
    for (int r = std::max((int)rows - 1, 0); r < row; r++)
    {
...
template <typename T>
std::size_t TableMatrix<T>::Rows() const {return rows;}
template <typename T>
std::size_t TableMatrix<T>::Cols() const {return cols;}

template <typename T>
T& TableMatrix<T>::Get(std::size_t row, std::size_t col) const {
    if (row < 0 || rows <= row || col < 0 || cols <= col)
    {
        throw std::invalid_argument("Index out of range");
    }
...
template <typename T>
T* TableMatrix<T>::operator[](std::size_t row) const {return data[row];}
...
template <typename T>
TableMatrix<T> TableMatrix<T>::operator+(const T& rhs) const {return *this + TableMatrix(rows, cols, rhs);}
template <typename T>
TableMatrix<T> TableMatrix<T>::operator-(const T& rhs) const {return *this - TableMatrix(rows, cols, rhs);}
template <typename T>
...
template <typename T>
bool TableMatrix<T>::operator!=(const TableMatrix<T>& rhs) const {return !(*this == rhs);}

template <typename T>
void TableMatrix<T>::Rebuild(std::size_t row, std::size_t col) {
    if (row + col > 0 && row * col == 0) {
        throw std::invalid_argument("Matrix must be square");
    }
