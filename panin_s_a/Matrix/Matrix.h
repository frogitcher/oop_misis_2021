...
template <typename T>
TableMatrix<T>::TableMatrix(size_t row, size_t col, const T& val) : BaseMatrix<T>(row, col)
{
    data = new T * [row];
    for (int r = 0; r < row; r++)
    {
...
{
    Rebuild(row, col);
    for (int r = std::max(rows - 1, (size_t)0); r < row; r++)
    {
        for (int c = std::max(cols - 1, (size_t)0); c < col; c++)
        {
            data[r * col + c] = val;
        }
    }
...
void TableMatrix<T>::Rebuild(size_t row, size_t col)
{
    ...
    for (int r = 0; r < row; r++)
        std::copy(data[r], data[r] + col, new_data[r]);
    for (int r = 0; r < rows; r++)
    {
        delete[] data[r];
        data[r] = nullptr;
