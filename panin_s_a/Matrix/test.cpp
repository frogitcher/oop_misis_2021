...
TEST_CASE("Testing TableMatrix constructors")
{
    ...
    CHECK(m3.Rows() == 5);
    CHECK(m3.Get(0, 0) == 113);
    CHECK_THROWS_WITH(TableMatrix<int>(0, 5), "if one dimention is zero other also should be zero");
    CHECK_THROWS_WITH(TableMatrix<int>({{1,2,3}, {1,2}, {12, 12, 12, 14}}), "number of elements in each row must be the same");
    CHECK_THROWS_WITH(m3.Get(-1, 0), "matrix index out of range");
